...
use syn::{
    ...
    token::Comma,
    DataStruct, DeriveInput, FieldsNamed, FieldsUnnamed, Ident, Path,
...
fn gen_clone_dyn_struct_fields(data: &DataStruct) -> TokenStream {
    match &data.fields {
...
fn gen_clone_dyn_struct_named_fields(fields: &FieldsNamed) -> TokenStream {
    let (fields_named, fields_named_clones) = make_named_fields(fields);
    quote!(match self {Self{#fields_named} => Self{#fields_named_clones} })
}
fn gen_clone_dyn_struct_unnamed_fields(fields: &FieldsUnnamed) -> TokenStream {
...
fn gen_clone_dyn_enum_variants(varaints: &Punctuated<Variant, Comma>) -> TokenStream {
...
fn make_clone_for_type(field_name: &Ident, field_type: &Type) -> TokenStream {
    match field_type {
...
fn make_named_fields(fields: &FieldsNamed) -> (TokenStream, TokenStream) {
...
fn make_unnamed_fields(fields: &FieldsUnnamed) -> (TokenStream, TokenStream) {
